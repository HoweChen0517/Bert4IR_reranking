...
class DenseBiEncoder(nn.Module):
    ...
    def score_pairs(self, queries, docs):
        ...
        """
        q_vectors = self.encode(queries.input_ids, queries.attention_mask)  # (batch_size, hidden_size)
        d_vectors = self.encode(docs.input_ids, docs.attention_mask)    # (batch_size, hidden_size)
        # calculate the dot product of each query-document pair
        scores = torch.bmm(q_vectors.unsqueeze(1), d_vectors.unsqueeze(2)).flatten()  # (batch_size, 1, hidden_size) * (batch_size, hidden_size, 1) = (batch_size, 1, 1) -> (batch_size,)
        return scores
